...
public class ClientThreadCoppy {
    ...
    public ClientThreadCoppy(Socket socket) {
        this.clientSocket = socket;
        new ClientThreadCoppy.ReadMsg().start(); // нить читающая сообщения из сокета в бесконечном цикле
        new ClientThreadCoppy.WriteMsg().start(); // поток пишуший в сокет
    }
            ...
            try {
                while (true) {
                    ...
                    if(!Objects.equals(message.getNickname(), "") || !Objects.equals(message.getText(), "") ||
                            !Objects.equals(message.getTime(), "") ){
                        String msg = "";
                        ...
                        if(message.getFile().length != 0) {
                            FileOutputStream fos = new FileOutputStream(System.getProperty("user.dir") +"\\files\\" + "i" + message.getText().substring(5));
    ...
    public class WriteMsg extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                Message m = new Message(reader.readLine(), TimeZone.getDefault().getID() , null , null);
            ...
            while (true) {
                try {
                    ...
                    if(s.startsWith("send ") && s.endsWith(" --file")){
                        ...
                        Message mmm = new Message(null , null,  s.substring(0 , s.length() - 7) , fileInArray);
                        ...
                        if (!s.equals("stop")) {
                            Message mm = new Message(null, null, s, null);
                            out.write(new Protocol(mm).getPacket());
                            out.flush();
                            //Message -> bytes; Получаем наш пакет: первые 3 байта: длина ника , длина времени , длина текста , дальше наше сообщение
                            // Отправляем пакет
                        } else {
                            out.write(new Protocol(new Message(null, null, "stop", null)).getPacket());
                            out.flush();
                            downService();
                            break;
                        }
